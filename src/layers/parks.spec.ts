...
describe('機能: 公園レイヤーを作る', () => {
  it('Given 表示ON / When 作る / Then 公園GeoJSONを塗りつぶし表示し、クリックで選択できる', () => {
    const layer = createParksLayer({ visible: true, selectedId: null, terrain: false })
  ...
  it('Given 選択中の公園 / When 色を決める / Then 選択中の公園は明るく塗る', () => {
    const layer = createParksLayer({ visible: true, selectedId: 'way/1', terrain: false })
...
describe('機能: 公園を地形に乗せる', () => {
  it('Given 地形あり / When レイヤーを作る / Then 標高ぶん持ち上げる拡張が付く', () => {
    const layer = createParksLayer({ visible: true, selectedId: null, terrain: true })
