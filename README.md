...
| 計測ログの重ね合わせ | 計測ログ（CSV）を読み込んで地点を地図に表示し、各地点の半径50・100・200 m以内の緑（街路樹の本数、区道の路線・本数、公園の数・面積、最寄りの公園までの距離）を数える。数えた列を足したCSVを書き出せる。データはこの端末の中だけで扱い、保存も共有リンクへの埋め込みもしない |
| 現在地ミニマップ | 東京23区全体の小さな地図を常時表示し、今の表示範囲をエメラルド色の矩形で重ねて示す。パン・ズーム・回転に追従する |

## 計測ログ（experiment_log.csv）の形式

Apple Watchなどで測った心拍変動（HRV）を場所ごとに記録したCSVを、アプリに読み込ませて、計測地点まわりの緑を数えられます。Apple Healthの書き出しには位置が入っていないので、場所の座標（`lat`・`lon`）は記録するときに1回だけ入れます。

```csv
timestamp,location,lat,lon,environment,hrv_sdnn,heart_rate,notes
2026-09-24 12:15,赤坂見附_ビル街,35.6795,139.7370,Urban,32,78,昼休み
2026-09-24 12:30,紀尾井町緑地,35.6787,139.7365,Green,58,68,日陰・ベンチ着席
```

- 必須列は `timestamp`・`location`・`lat`・`lon`。`environment`（Green / Urban）・`hrv_sdnn`・`heart_rate`・`notes` は任意です。列名で判定するので並び順は問いません。ほかの列は書き出し時にそのまま残します
- 緯度経度が数値でない行や、東京付近の範囲を外れる行（緯度と経度の取り違えなど）は、行番号つきで知らせて読み飛ばします
- 「緑の文脈つきCSVを書き出す」で、元の列の後ろに `radius_m, trees_all, trees_tall, city_tree_routes, city_trees_known, parks_count, parks_area_m2, nearest_park_m, in_park` が付きます。pandasなどにそのまま渡せます
- 書き出しは UTF-8（BOMなし）です。Excelで開くと文字化けする場合は、データの取り込み機能でUTF-8を指定してください
...
src/
  ...
  data/greenContext.ts        計測地点まわり（半径R m）の緑の集計
  lib/measurementLog.ts       計測ログCSVの読み込みと、緑の文脈つきCSVの書き出し
...
- **計測ログの扱い**：HRVなどの健康データを含むため、読み込んだ計測ログはメモリにだけ置きます。localStorageへの保存も、共有リンクへの埋め込みもしません（再読み込みで消えます）。書き出しはブラウザのダウンロードで、どこにも送信しません
- **計測地点まわりの緑の数え方**：樹種・行政区の絞り込みに関係なく全件で数えます（画面の状態で数値が変わらないようにするため）。区道の路線と公園は、円に一部でも重なれば全体（本数・面積）を数えるので、円の端では実際より多めになります。円と公園が重なる部分の面積は求めていません。地図データ（街路樹・区道・公園）の読み込みが終わるまで、緑の文脈は計算せず、書き出しも押せません。区道・公園の読み込みに失敗した場合も同じ状態のままになります
